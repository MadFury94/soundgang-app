...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!artist) return buildMetadata({ title: 'Artist Not Found', noIndex: true });
    return buildMetadata({
        title: artist.name,
        description: artist.bio,
        image: artist.coverImage || artist.image,
        url: `https://soundgang.ng/artists/${artist.slug}`,
        type: 'profile',
    });
}
...
export default async function ArtistPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // Fetch from API (falls back to static store if API unavailable)
    const storeArtist = await getArtistBySlug(slug);
    ...
    const jsonLd = buildArtistJsonLd({
        name: artist.name,
        bio: artist.bio,
        image: artist.image || artist.coverImage,
        slug: artist.slug,
        genre: artist.genre,
        social: { spotify: artist.social.spotify, instagram: artist.social.instagram, twitter: artist.social.twitter },
    });
